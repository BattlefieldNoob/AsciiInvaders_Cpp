	...
	if(GameOver){
		...
		for(EnemyIt=enemys.begin();EnemyIt<enemys.end();EnemyIt++){
			for(RocketIt=rockets.begin();RocketIt<rockets.end();RocketIt++){
				if((*RocketIt)->Y()<0){
					RocketIt=rockets.erase(RocketIt);//Rocket is out of screen
				}
				else{
					if((int)(*RocketIt)->Y() == (int)(*EnemyIt)->Y()){//check collision between Rocket and enemy
						if(((int)(*RocketIt)->X())>=((int)(*EnemyIt)->X()) && ((int)(*RocketIt)->X())<=((int)(*EnemyIt)->X())+2){
							EnemyIt=enemys.erase(EnemyIt);//Delete Enemy
							RocketIt=rockets.erase(RocketIt);//Delete Rocket
						}
					}
				}
			}
...
void BattleField::playerLeft(){if(player->X()>1){
